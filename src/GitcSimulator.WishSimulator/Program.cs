namespace GitcSimulator.WishSimulator
{
	public class Program
	{
		...
		{
			SimulateStatistics();
			//SimulateHowManyWishesItTakes();
		}

		private static void SimulateHowManyWishesItTakes()
		{
			var result = PerformSimulation(
				c => c.Cons5StarsPromo < 6
			);
			Console.WriteLine($"Took {result.Wishes} to get the desired. In the mean time you got:");
			...
			Console.WriteLine($"4Star Wins: {result.Wins4Stars}");
			Console.WriteLine($"4Star Loss: {result.Loss4Stars}");
			Console.WriteLine($"Captured radiances: {result.RadianceCaptured}");
		}

		private static void SimulateStatistics()
		{
					       ...
					       radianceCaptured)))
			{
				...
				if (Wish(5.1) || currentBatch4Star >= 10)
				{
					if (guarantee4Star)
					{
						...
						var ch = Random.Next(0, chars4Star.Length);
						chars4Star[ch]++;
		...
		private class WishSimulationResult
		{
				...
				int radianceCaptured)
			{
				...
				RadianceCaptured = radianceCaptured;
			}
