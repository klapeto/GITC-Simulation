...
namespace GitcSimulator.Core.Values
{
	public class StatEffect<T> : IEffect
	{
		...
		private readonly Func<Attributes, IAttribute<T>> _statGetter;

		public StatEffect(Func<Attributes, IAttribute<T>> statGetter, T? flatModifier, Percent? percentModifier)
		{
		...
		{
			var stat = _statGetter(lifeform.Attributes);
		...
		{
			_statGetter(lifeform.Attributes).Remove(_id);
		}
